# Comments were already written to file. Comments with "##" are mine.
...
def add_to_inventory(inventory, added_items):
    ...
    ## We have to update now, otherwise later the dictionary will not show added items.
...
def print_table(inventory, order=None):             
    ...
    sum_of_inventory = 0
    ## We distinguish longest key and longest value so, if one of them is abnormaly long, the other column will remain the same.
    longest_key_from_dictionary = len(max(inv_copy, key=len))
    longest_value_from_dictionary = len(str(max(inv_copy.values())))
    ## Extra space between columns for better view.
    extra_space = 5
    ## One extra dash for perfect fit.
    alignment_for_dash_series = 1
    print("Inventory:")
    print("count".rjust(longest_value_from_dictionary + extra_space), "item name".rjust(longest_key_from_dictionary + extra_space))
    print("-" * (longest_key_from_dictionary + longest_value_from_dictionary + 2 * extra_space + alignment_for_dash_series))
    for key, value in inv_copy.items():
        print(str(value).rjust(longest_value_from_dictionary + extra_space), str(key).rjust(longest_key_from_dictionary + extra_space))
        sum_of_inventory += value
    print("-" * (longest_key_from_dictionary + longest_value_from_dictionary + 2 * extra_space + alignment_for_dash_series))
    print("Total number of items:", sum_of_inventory, "\n")
